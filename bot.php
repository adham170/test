<?php
ob_start();
$API_KEY = "1456131990:AAE1K9shHzxOAodCK9cXClfZ2l-hExj2XUw";
$buy = "@Q_Y_4";
$sudo = "1488653639";
$admin = "1488653639";
$idBot = "1456131990";
$admmm = 1488653639;
$Dev = array("1488653639", "" );
$chanle  = "t.me/FAQY4";
$admin2  = "t.me/Q_Y_4";
$userrr = "FLASH_XBOT";
...
function bot($method,$Q_Y_4=[]){
$Q_Y_4 = http_build_query($Q_Y_4);
$url = "https://api.telegram.org/bot".API_KEY."/".$method."?$Q_Y_4";
$Q_Y_4 = file_get_contents($url);
return json_decode($Q_Y_4);}
...
if($text == "تفعيل" or $text == "ايدي" or $text == "سورس" or $text == "الاوامر" or $text == "م1" or $text == "م2" or $text == "م3" or $text == "م4" or $text == "م5" or $text == "السورس" or $text == "الاعدادات" or $text == "رتبتي" or $text == "كشف" or $text == "اوامر الرد" or $text == "تحديث" or $text == "تحديث السورس" or $text == "تحديث سورس" and $re_id != "$admin"){
$ch = FAQY4;
$from_id = $message->from->id;
$join = file_get_contents("https://api.telegram.org/bot".API_KEY."/getChatMember?chat_id=@FAQY4&user_id=".$from_id);
...
elseif($re and $text == "رفع مدير" || $text == "رفع المدير" || $text == "رفع منشئ" || $text == "رفع المنشئ" and in_array($re_id,$manger) and   $status == "creator" ||  in_array($from_id,$Dev) || in_array($from_id,$developer)){
bot('SendMessage',['chat_id'=>$chat_id,
'text'=>"
¦ العضو » [$usew]
¦ الايدي » {[$re_id]}
...
if($re and $text == "تنزيل المدير" || $text == "تنزيل مدير"  and in_array($re_id,$manger) and $status == "creator" ||  in_array($from_id,$Dev) || in_array($from_id,$developer)){
...
if($re and $text == "تنزيل المدير" || $text == "تنزيل مدير" || $text == "تنزيل منشئ" || $text == "تنزيل المنشئ" and !in_array($re_id,$manger) and $status == "creator" ||  in_array($from_id,$Dev) || in_array($from_id,$developer)){
bot('SendMessage',['chat_id'=>$chat_id,
'text'=>"
¦ العضو » [$usew]
¦ الايدي » {[$re_id]}
...
if($text == "المدراء" || $text == "قائمه المدراء" and $mangers_info != NULL and $mangers_info != " " and $status == "creator" ||  $status == "administrator" ||  in_array($from_id,$Dev) || in_array($from_id,$developer)){
bot('SendMessage',['chat_id'=>$chat_id,
'text'=>"*📋¦ قائمه المدراء :*
$mangers_infos\n",'parse_mode'=>'markdown','reply_to_message_id'=>$message->message_id,'disable_web_page_preview'=>true,
]);
...
if($text == "المدراء" ||  $text == "قائمه المدراء" and $mangers_info == NULL || $mangers_info == " " || $mangers_info == ""  and $status == "creator" ||  $status == "administrator" ||  in_array($from_id,$Dev) || in_array($from_id,$developer)){
bot('SendMessage',['chat_id'=>$chat_id,
'text'=>"*📋¦ قائمه المدراء :
...
elseif($re and $text == "رفع ادمن" || $text == "رفع ادمن" || $text == "رفع منشئ"  and in_array($re_id,$admin_user) || $status == "creator" ||  $status == "administrator" ||  in_array($from_id,$Dev) || in_array($from_id,$developer)){
bot('SendMessage',['chat_id'=>$chat_id,
'text'=>"
¦ العضو » [$usew]
¦ الايدي » {[$re_id]}
...
if($text == "مسح الادمنيه"  and $status == "creator" ||  $status == "administrator" ||  in_array($from_id,$Dev) || in_array($from_id,$developer)){
...
if($re and $text == "تنزيل ادمن" and in_array($re_id,$admin_user) || $status == "creator" ||  $status == "administrator" ||  in_array($from_id,$Dev) || in_array($from_id,$developer)){
...
if($re and $text == "تنزيل ادمن"  and !in_array($re_id,$admin_user)  and $status == "creator" ||  $status == "administrator" ||  in_array($from_id,$Dev) || in_array($from_id,$developer)){
bot('SendMessage',['chat_id'=>$chat_id,
'text'=>"
¦ العضو » [$usew]
¦ الايدي » {[$re_id]}
...
elseif($re and $text == "رفع مميز"  and in_array($re_id,$mmyaz)  and $status == "creator" ||  $status == "administrator" ||  in_array($from_id,$Dev) || in_array($from_id,$developer)){
bot('SendMessage',['chat_id'=>$chat_id,
'text'=>"
¦ العضو » [$usew]
¦ الايدي » {[$re_id]}
...
if($text == "مسح المميزين"  and $status == "creator" ||  $status == "administrator" ||  in_array($from_id,$Dev) || in_array($from_id,$developer)){
...
if($re and $text == "تنزيل مميز"   and in_array($re_id,$mmyaz)  and $status == "creator" ||  $status == "administrator" ||  in_array($from_id,$Dev) || in_array($from_id,$developer)){
...
if($text == "المميزين" || $text == "قائمه المميزين" and $mmyazs_info != NULL and $mmyazs_info != " "  and $status == "creator" ||  $status == "administrator" ||  in_array($from_id,$Dev) || in_array($from_id,$developer)){
bot('SendMessage',['chat_id'=>$chat_id,
'text'=>"*📋¦ قائمه المميزين :*
$mmyazs_infos\n",'parse_mode'=>'markdown','reply_to_message_id'=>$message->message_id,'disable_web_page_preview'=>true,
]);
}
if($text == "المميزين" ||  $text == "قائمه المميزين" and $mmyazs_info == NULL || $mmyazs_info == " " || $mmyazs_info == ""  and $status == "creator" ||  $status == "administrator" ||  in_array($from_id,$Dev) || in_array($from_id,$developer)){
bot('SendMessage',['chat_id'=>$chat_id,
'text'=>"*📋¦ قائمه المميزين :
...
$message_idd = $update->message->message_id;
if($text == "ترقية رئيس"  || $text == "رفع رئيس"){
bot('SendMessage',['chat_id'=>$chat_id,
'text'=>"
¦ العضو » [@$re_user](tg://user?id=$re_id)
¦ الايدي » {*`$re_id`*}
...
$message_idd = $update->message->message_id;
if($text == "ترقية وزير"  || $text == "رفع وزير" ){
bot('SendMessage',['chat_id'=>$chat_id,
'text'=>"
¦ العضو » [@$re_user](tg://user?id=$re_id)
¦ الايدي » {*`$re_id`*}
...
$message_idd = $update->message->message_id;
if($text == "ترقية شرطي"  || $text == "رفع شرطي"){
bot('SendMessage',['chat_id'=>$chat_id,
'text'=>"
¦ العضو » [@$re_user](tg://user?id=$re_id)
¦ الايدي » {*`$re_id`*}
...
$message_idd = $update->message->message_id;
if($text == "رفع مساعد"  || $text == "ترقية مساعد"){
bot('SendMessage',['chat_id'=>$chat_id,
'text'=>"
¦ العضو » [@$re_user](tg://user?id=$re_id) 
¦ الايدي » {*`$re_id`*}
...
$message_idd = $update->message->message_id;
if($text == "رفع امير"  || $text == "رفع اميره" ){
bot('SendMessage',['chat_id'=>$chat_id,
'text'=>"
¦ العضو » [@$re_user](tg://user?id=$re_id)
¦ الايدي » {*`$re_id`*}
...
$message_idd = $update->message->message_id;
if($text == "رفع مؤدبه"  || $text == "رفع مزه" ){
bot('SendMessage',['chat_id'=>$chat_id,
'text'=>"
👤¦ العضو » [@$re_user](tg://user?id=$re_id)
*🎫¦ الايدي » {*`$re_id`*}
...
",'parse_mode'=>'markdown','reply_to_message_id'=>$message->message_id,'disable_web_page_preview'=>true,
 ...
 $message_idd = $update->message->message_id;
if($text == "رفع مقوت"  || $text == "رفع مقوتي" || $text == "ترقية مقوت" || $text == "رفع مخزن"){
bot('SendMessage',['chat_id'=>$chat_id,
'text'=>"
¦ العضو » [@$re_user](tg://user?id=$re_id)
¦ الايدي » {*`$re_id`*}
...
$message_idd = $update->message->message_id;
if($text == "رفع مجنون"  || $text == "رفع مجنونه"){
bot('SendMessage',['chat_id'=>$chat_id,
'text'=>"
¦ العضو » [@$re_user](tg://user?id=$re_id) 
¦ الايدي » *`$re_id`*
...
$Free1 = array("نورت المجموعة","ماشاء الله نورت","العبب","عليك نور","صورة جميله","متغيرهاش","جبتها منين","اوف عاجمدان","مش حلوه","غيرها ","حطمت القلوب","عمري انت","صدقني روعه","بقت قديمه","ممكن صورة","الله عليك","افضل صورة","مش عجباني","رووووعاتك","والله فشيخه ","Very Nice ","لاتكثر الايدي","ذوقك فن");
...
if($msgs['msgs'][$chat_id][$from_id] > 3000){
$active = array("جامد في التفاعل 🌝","متفاعل ✨","اسطورة التفاعل 🌈ء","الله عالتفاعل ⚜","نايس التفاعل 💘ء",'قوي جدا ⚡️ ',  'قمه التفاعل ✨ ',  'اقوى تفاعل 🔥 ',);
...
if(!$rep and $text=="ايدي" || $text == "id" || $text == "/id"){
$result=json_decode(file_get_contents("https://api.telegram.org/bot".API_KEY."/getUserProfilePhotos?user_id=$from_id"),true);
$file_id=$result["result"]["photos"][0][0]["file_id"];
$count=$result["result"]["total_count"];
  var_dump(
...
╽رتبتـــك ⇜ $info ⊰•
╽موقعك ⇜ $momo ⊰•
╽تفاعـلك ⇜ $active[$JJ119] $Free3[$Free4] ⊰• 
╽عدد صـورك ⇜ $count ⊰• 
╽تعديلاتك ⇜ $editt ⊰•
...
elseif( $text  == "المحظورين عام" || $text == "العام") {
if ( in_array($from_id,$Dev)) {
$silent = $user["banlist"];
...
// lock photo
 ...
 elseif($text  == "/silent"&& $rt or $text  == "/mute" && $rt or $text  == "تقييد" && $rt){
if ( $status == 'creator' or $status == 'administrator' or in_array($from_id,$Dev) or in_array($from_id,$manger) or in_array($from_id,$admin_user) or in_array($from_id,$developer)) {
...
elseif($text  == "/unsilent" && $rt or $text  == "/unmute" && $rt or $text  == "الغاء التقييد" && $rt){
if ( $status == 'creator' or $status == 'administrator' or in_array($from_id,$Dev) or in_array($from_id,$manger) or in_array($from_id,$admin_user) or in_array($from_id,$developer)) {$add = $settings["information"]["added"];
if ($add == true) {
 bot('restrictChatMember',[
   'user_id'=>$re_id,   
...
bot('sendMessage',[
'chat_id'=>$chat_id, 
'text'=>"*آسمـي $namebot 🌚❤️.*"
,'parse_mode'=>"MARKDOWN",
 'reply_to_message_id'=>$message_id,
]);}}
$message = $update->message;
$h4ck3riraq = array('قلب البوت 🌚👌',
'ممق قرعه',' مشغول 🦁', 'اووف طلعت روحي ','صلي على النبي 🌿',
);
if($text == "$namebot"){
...
'parse_mode'=>'MarkDown', 'disable_web_page_preview'=>true,
  ...
  'reply_markup'=>json_encode([
'inline_keyboard'=>[
[['text'=>"ﺂ لمطـــــور 🍁",'url'=>"t.me/Q_Y_4"]],
]])
]);
}}}}
...
ـ••┉┉┉┉┉┉┉┉┉┉┉┉┉••
**📯┊للمزيد تابع قناة البوت [source](https://t.me/FAQY4) **
",
'parse_mode'=>'MarkDown',
'reply_to_message_id'=>$message_id,
'reply_markup'=>json_encode([
...
➖➖➖➖
 ...
 elseif($text =="📜 عرض المجموعات" || $text=="المجموعات" ){
if ($tc == "private") {
if (in_array($from_id,$Dev)) {
	bot('senddocument',[
	'chat_id'=>$chat_id,
...
elseif(  $text =="الغاء التثبيت" || $text == "الغاء تثبيت"){
...
'reply_markup'=>$inlinebutton,
   ...
   //del
elseif( $rt && $text  == "حذف" || $rt and $text=="مسح"){
...
»",
  'reply_to_message_id'=>$message_id,
    ...
    'inline_keyboard'=>[
 	[
['text'=>"$namebot 🌚",'url'=>"https://t.me/FAQY4"]
	  ],
   ]
   ])
     ]); 
...
",
  'reply_to_message_id'=>$message_id,
    ...
    'inline_keyboard'=>[
	 	[
['text'=>"القناه",'url'=>"https://t.me/FAQY4"]
	  ],
   ]
   ])
     ]); 
...
$chat_id     = $message->chat->id;
$user          = $update->message->from->username;
 $sudo         = 1642781423; // ايديك.
$bot_id       = 1623800506; // ايدي بوتك .
$from_id     = $message->from->id;
$first_name = $message->from->first_name;
$type       = $update->message->chat->type;
mkdir("Fri3nd_s");
...
if($text =="السورس" || $text =="سورس"){
	...
	'chat_id'=>$chat_id,
	'text'=>"*┄─┅══┅─┄     
👋│اهلا بك في سورس فلاش ⚜
●ـ▬ـ▬ஜ۩۞۩ஜ▬ـ▬ـ●
📡╿*[قناة السورس العامة](t.me/FAQY4)*
...
'chat_id'=>$chat_id,
'text'=>"*🎖
🗂¦ تم تحديث ملفات السورس بنجاح √
🔘¦ للمزيد»* [اضغط هنا](t.me/FAQY4)*❕
▂
*",
'parse_mode'=>'MarkDown', 'disable_web_page_preview'=>true, 'reply_to_message_id'=>$message->message_id,
]);
...
'disable_web_page_preview'=>true,
        ...
        'reply_markup'=>json_encode([
          'inline_keyboard'=>[  
                [['text'=>'- تابع قناة السورس ✅','url'=>'https://t.me/FAQY4']],
               ]
        ])
    ]);
}
...
$twit = array('تخطط للمستقبل ولا ؟','جربت شعور احد يحبك بس انت مو قادر تحبه؟','كلمة لشخص بعيد؟','تتابع انمي؟ إذا نعم ما أفضل انمي شاهدته','اسم ماتحبه ؟','قهوه و لا شاي؟','ما هي وظفتك الحياه','ما السيء في هذه الحياة ؟','اخر كتاب قرآته','عمرك ضحيت باشياء لاجل شخص م يسوى ؟','أكثر تغيير ترغب أن تغيّره في نفسك؟','تاريخ ميلادك؟','كثر طبع تهتم بأن يتواجد في شريك/ة حياتك؟','فيلم عالق في ذهنك لا تنساه مِن روعته؟','كلمات ماتستغني عنها بسوالفك؟','هل التعود على شخص والتحدث معه بشكل يومي يعتبر نوع من أنواع الحب؟','هل أنت من النوع الذي يواجه المشاكل أو من النوع الذي يهرب ؟','كيف تتعامل مع الشخص المُتطفل ( الفضولي ) ؟','لي انت مش سالك ؟',' كم تحتاج من وقت لتثق بشخص؟',' وش الحب بنظرك؟','  متى يصبح الصديق غريب',' حيوانك المفضل؟','  يهمك ظن الناس فيك ولا ؟','-› مسلسل كرتوني له ذكريات جميلة عندك؟',' هل التعود على شخص والتحدث معه بشكل يومي يعتبر نوع من أنواع الحب؟','  انت حزين اول شخص تتصل عليه؟','›  شيء تعترف انك فاشل فيه ؟','كلمتك التسليكيه ؟','أكثر سبب منطقي يمكن أن يجعلك تقرر الإبتعاد عن الشخص الذي تُحب؟','افضل هديه ممكن تناسبك؟','من الاشياء اللي تجيب لك الصداع ',' كلمتين تكررها دايم ؟!','كتاب أو رواية تقرأها هذه الأيام؟','أكثر شيء أسعدك اليوم؟','شاركنا أقوى بيت شِعر من تأليفك؟',' اكثر كلمة تنقال لك بالبيت ؟','جربت شعور احد يحبك بس انت مو قادر تحبه؟');
$twit2 = array_rand($twit,1);
...
💬 ¦•⊱ تويت / لعبه اساله ممتعه لملئ الفراغ
ـــ ـــ ـــ ــــ ــــ
...
if($text =="السورس" || $text =="سورس"){
	...
	'chat_id'=>$chat_id,
	'text'=>"
    مرحبا بك في سورس فلاش
",
   'reply_markup'=>json_encode([
    'inline_keyboard'=>[
	[['text'=>"√ • المطور• √",'url'=>"$admin2"]
